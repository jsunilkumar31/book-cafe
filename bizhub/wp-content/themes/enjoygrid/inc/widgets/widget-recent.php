 ...
 * @since      1.0.0
 */
class enjoygrid_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-enjoygrid-recent widget-posts-thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'enjoygrid' )
		);

		// Create the widget.
		parent::__construct(
			'enjoygrid-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'enjoygrid' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'enjoygrid_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'enjoygrid_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'enjoygrid_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'enjoygrid' ),
			'limit'     => 5,
			'show_date' => true
		);
