 ...
 * @since      1.0.0
 */
class EnjoyGrid_Most_Commented_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-enjoygrid-most-commented widget-posts-thumbnail',
			'description' => __( 'Display most commented posts with thumbnails.', 'enjoygrid' )
		);

		// Create the widget.
		parent::__construct(
			'enjoygrid-most-commented', // $this->id_base
			__( '&raquo; Most Commented Posts', 'enjoygrid' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'enjoygrid_most_commented_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'enjoygrid_most_commented_widget_' . $this->id, $popular );
	...
	function flush_widget_transient() {
		delete_transient( 'enjoygrid_most_commented_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Most Commented Posts', 'enjoygrid' ),
			'limit'     => 5,
			'show_date' => true
		);
	...
	?>
		...
		<p>
			...
			<label for="<?php echo esc_html( $this->get_field_id( 'show_date' ) ); ?>">
				<?php esc_html_e( 'Display post date?', 'enjoygrid' ); ?>
			</label>
		</p>
