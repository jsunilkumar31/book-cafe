 ...
 * @since      1.0.0
 */
class enjoygrid_Random_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-enjoygrid-random widget-posts-thumbnail',
			'description' => __( 'Display random posts with thumbnails.', 'enjoygrid' )
		);

		// Create the widget.
		parent::__construct(
			'enjoygrid-random',                                   // $this->id_base
			__( '&raquo; Random Posts', 'enjoygrid' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $random = get_transient( 'enjoygrid_random_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'enjoygrid_random_widget_' . $this->id, $random );
	...
	function flush_widget_transient() {
		delete_transient( 'enjoygrid_random_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Random Posts', 'enjoygrid' ),
			'limit'     => 5,
			'show_date' => true
		);
