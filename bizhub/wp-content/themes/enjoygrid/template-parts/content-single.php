	...
	<div class="entry-content">
		<?php
			the_content( sprintf(
				...
				wp_kses( __( 'Continue reading %s <span class="meta-nav">&rarr;</span>', 'enjoygrid' ), array( 'span' => array( 'class' => array() ) ) ),
				the_title( '<span class="screen-reader-text">"', '"</span>', false )
			) );

			wp_link_pages( array(
				'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'enjoygrid' ),
				'after'  => '</div>',
			) );
		?>
	</div><!-- .entry-content -->
...
<?php
	...
	$args = apply_filters( 'enjoygrid_related_posts_args', $query );
	...
	if ( $related->have_posts() ) : $i = 1; ?>
		...
		<div class="entry-related clear">
			<h3><?php esc_html_e('Related Posts', 'enjoygrid'); ?></h3>
						...
						<?php if ( has_post_thumbnail() ) { ?>
							<a class="thumbnail-link" href="<?php the_permalink(); ?>">
								<div class="thumbnail-wrap">
									<?php 
										the_post_thumbnail('enjoygrid_post_thumb'); 
									?>
								</div><!-- .thumbnail-wrap -->
							</a>
						<?php } ?>				
